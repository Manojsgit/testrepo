...
int main()
{
	...
	int start_row, position_valid, input_valid = 0, stalemate_result = 0;
	...
	while (1) {
		...
		while (1) {
			...
			stalemate_result = check_stalemate(board, white_row, white_col, black_row, black_col, user_check);
			break;
		}
		if (stalemate_result == ZERO)
			break;
		user = user + 1;
...
{
	char opp_pawn;
	int i, j, not_movablepawn = 0, *row_comb, *col_comb, result, increment = 0;
	...
	for (i = 0; i < 8; i++) {
		for (j = 0; j < 8; j++) {
			if (board[i][j] == opp_pawn) {
				...
				result = possible_moves(board, user, i + ONE, j + ASCI, row_comb, col_comb);
				if (result == ZERO)
